...
-   starts_with()
    ...
    ```{r}
    View(select(flights, endd_with("ar")))
    ```
...
summarize(by_day, delay = mean(dep_delay, nm.rm = TRUE))
...
not_cancelled <- flight %>%
  flight(!is.na(dep_delay), !is.na(arr_delay))
...
ggplot(data = delays, mapping = aes(x = n, y = dealy)) + 
